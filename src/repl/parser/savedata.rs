use crate::calendar::{CalendarReference, Calendar, self};
...
pub fn save_event(event: Event, calendar_ref: CalendarReference) {
	...
	Ok(result) => result,
	Err(e) => {
	    println!("Failed to parse {}. Check for syntax errors.\n{}", calendar_ref.path, e);
	    return
	}
    };
    ...
    let mut new_file = match std::fs::OpenOptions::new().write(true).truncate(true).open(&calendar_ref.path) {
	Ok(file) => file,
	Err(e) => {
