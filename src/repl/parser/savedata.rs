use crate::calendar::{Calendar, CalendarReference, CalendarIndex};
...
pub fn save_calendar_index(calendar_index: CalendarIndex) {
    ...
    let mut index_file = match std::fs::OpenOptions::new().write(true).truncate(true).open(home_dir.join(".config/calmar/index.json")) {
	Ok(file) => file,
	Err(e) => {
	    println!("Failed to open {}.\n{}", home_dir.join(".config/calmar/index.json").display(), e);
	    std::process::exit(1);
	}
    };
...
pub fn save_new_calendar(calendar_reference: CalendarReference) {
    ...
    let calendar_json: String = match serde_json::to_string_pretty(&Calendar::new(&calendar_reference.name)) {
	Ok(result) => result,
	Err(e) => {
