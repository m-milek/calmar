...
pub fn print_err_msg(err: CalmarError, info: &String) {
    match err {
        CalmarError::ReadFile { e } => {
	    error(format!("Failed to read {} \n{}", info, e))
	},
        CalmarError::ParseJSON { e } => {
	    error(format!("Failed to parse {} as JSON.\n{}",info, e))
	},
	...
	CalmarError::ToJSON { e } => {
	    error(format!("Failed to serialize struct to JSON.\n{:#?}\n{}", info, e))
	},
	CalmarError::ActiveCalendarCount { e } => {
	    error(format!("There are {} calendars set as 'active'. There should be exactly one.", e))
	}
    }
}
